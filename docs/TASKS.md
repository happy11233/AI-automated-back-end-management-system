...
- [x] 基于真实运行记录和审计事件生成效果指标
- [x] 新增效果分析页面 `/effect-analytics`
- [x] 展示自动化次数、成功率、失败率、越权拦截、审计审批事件、ERP/AI 应用使用和节省时间估算
- [x] 按岗位、应用、运行类型展示使用排行与失败/拦截原因
- [x] 员工效果分析强制限定为本人和当前岗位，不展示跨岗位统计
- [x] 新增 `GET /effect-analytics` 只读接口
- [x] 新增真实 API 验证脚本 `scripts/verify_effect_analytics.py`
- [x] 新增真实浏览器验证脚本 `scripts/verify_effect_analytics_frontend.mjs`
- [x] 页面布局通过管理员桌面、财务移动端和员工权限截图检查
